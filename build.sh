...
lunching() {
  ...
  source build/envsetup.sh
  lunch havoc_$DEVICE-$BUILD_TYPE &>lunch_log.txt
  ...
  if grep -q "dumpvars failed with" lunch_log.txt; then
    ...
    send_msg "<b>🛠 CI | Havoc-OS rev $PROJECT_VERSION</b>%0A<b>📲 Device:</b> <code>$DEVICE</code>%0A<b>🛡 Type:</b> <code>$BUILD_TYPE</code>%0A<b>☠️ Cores:</b> <code>$JOBS</code>%0A<b>🌚 ID:</b> <code>$BUILD_NUMBER</code>"
    building
  fi
}
...
build_status() {
  ...
  BUILD_PACKAGE="$(find "$OUT_DIR" -name "Havoc-OS-*.zip" -print -quit)"
